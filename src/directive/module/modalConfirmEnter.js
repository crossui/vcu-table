...
const bindEvent = (el, binding, vnode) => {
  el._modalEnterHandler = _.debounce(async (e) => {
    ...
    if (modals.length && modals[0]) {
      ...
      let childWrap = currModal.getElementsByClassName("ant-modal-wrap");
      ...
      let btns = currModal.getElementsByClassName('ant-btn');
      let index = await returnCurrDom(btns);
      let wrapper = currModal.getElementsByClassName('ant-modal-confirm-body-wrapper')
      if (e.keyCode == 13) {
        //回车键
        ...
        if (btns.length > 1 && index !== false) {
          btns[index].classList.remove("ant-btn-primary");
          if (index - 1 < 0) {
            btns[index + 1].classList.add("ant-btn-primary");
  ...
  return new Promise((resolve, reject) => {
    try {
      Array.prototype.forEach.call(btns, (element, index) => {
        if (hasClass(element, 'ant-btn-primary')) {
          resolve(index)
        }
      });
    } catch (err) {
